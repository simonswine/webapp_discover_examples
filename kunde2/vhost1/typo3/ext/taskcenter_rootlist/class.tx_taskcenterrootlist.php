...
class tx_taskcenterrootlist extends mod_user_task {
	...
	function overview_main(&$pObj)	{
		if ($this->BE_USER->isAdmin())	{
			$headContent='<a href="'.$this->backPath.'db_list.php?id=0" target="list_frame">'.htmlspecialchars($pObj->MOD_MENU["function"]["tx_taskcenterrootlist"]).'</a>';
			$content.=$pObj->doc->section($headContent,"",1,1,0,1);
		}
		return $content;
	}
	function main() {
