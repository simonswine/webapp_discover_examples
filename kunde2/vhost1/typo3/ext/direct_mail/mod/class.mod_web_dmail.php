...
//		$theOutput.= $GLOBALS["SOBE"]->doc->spacer(20);
	...
	function cmd_displayPageInfo()	{
		...
		if (is_array($_POST["indata"]["categories"]))	{
			$data=array();
			reset($_POST["indata"]["categories"]);
			while(list($recUid,$recValues)=each($_POST["indata"]["categories"]))	{
//						debug($recValues);
				reset($recValues);
...
//				debug($mailGroup);
	...
	function cmd_query($dgUid)	{
		...
		$GLOBALS["SOBE"]->MOD_SETTINGS["dmail_queryConfig"]=serialize($_POST["dmail_queryConfig"]);
		$GLOBALS["SOBE"]->MOD_SETTINGS["dmail_queryTable"]=$_POST["SET"]["dmail_queryTable"];
...
//					debug($_POST["indata"]["categories"]);

					$data[$table][$uid]["module_sys_dmail_html"] = $_POST["indata"]["html"] ? 1 : 0;
					$tce = t3lib_div::makeInstance("t3lib_TCEmain");
					$tce->stripslashes_values=0;
					$tce->start($data,Array());
					$tce->process_datamap();
